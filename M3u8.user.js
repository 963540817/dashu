...
// @include /^https?:\/\/(?:(?!\d+?\/)[^\/]+?\/){1,}(?:\d+?\/?){1,}$/
// @include /^https?:\/\/[^\/]+?\/{1,}[a-zA-Z]{4,8}\?[a-zA-Z]{4,8}=[^\/\?\&\.]+?$/
// @include /^https?:\/\/[^\/]+?\/{1,}(?:[pP]lay|[sS]hare)\/{1,}[a-zA-Z0-9]+?\/?\s*?$/
// @include /^https?:\/\/[^\/]+?\/{1,}(?:\d{4,8}\.s?htm|[a-zA-Z\d]+?\/\d{4,8}\.s?html?\?id=\d{4,8}$)/
// @include /^https?:\/\/[^\/]+?\/{1,}(?:(?:(?:bf|[mM][pP]4|[vV](?:ideos?)?)\/(?:[^\/]+?\/?|\d{4,8}\/[^\/\.\?\=]+?)|\d{4,8}(?:\/?|\/\d+?\/?))|[^\/]+?\/)?$/
        ...
        if (typeof location['m3u8去插播广告'] === 'undefined') {
            ...
            try {
                if (typeof Proxy === 'function') {
                    ...
                    ggbmd = /\.php|\/\?vkey=|(?:[\.\_\/](?:ts|mp4|flv)(?:\/.+?)?\.m3u8|\/m3u8\/.*?m3u8[\?&].+?\.m3u8.*?)(?:#|\?|\\|&|$)|\.m3u8\?(?:[a-z]=[^=]+?=$|[^_]+?_key=)/i;
